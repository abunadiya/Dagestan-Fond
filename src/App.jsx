...
const translations = {
  ru: {
    ...
    footerAboutTitle: 'О фонде',
    footerAboutText:
      'Некоммерческая ассоциация, объединяющая выходцев из Дагестана во Франции. Мы оказываем социальную поддержку, развиваем культурно-образовательные программы и сохраняем наследие нашего народа.',
  ...
  fr: {
    ...
    footerAboutText:
      'Association à but non lucratif réunissant les ressortissants du Daghestan en France. Nous apportons un soutien social, développons des programmes culturels et éducatifs, et préservons l\'héritage de notre peuple.',
...
function LangSwitch({ lang, onChange, className = '' }) {
  return (
    <div className={`df-lang-switch ${className}`.trim()} role="group" aria-label="Language">
      <button
        type="button"
...
function App() {
  ...
  const navLinks = [
    ...
    { id: 'alamana', label: t.navAlAmana },
    { id: 'contacts', label: t.navContacts },
  ];

  return (
    <>
      <nav className="navbar df-navbar sticky-top">
        <div className="container">
          <a className="navbar-brand" href="#top" onClick={() => setIsOpen(false)}>
            <img src={logo} alt="DAGESTAN - FOND" className="df-logo" />
            <span className="df-brand-title font-display">{t.navBrand}</span>
          </a>

          <div className="df-navbar-actions">
            <LangSwitch lang={lang} onChange={setLang} className="df-lang-switch--desktop" />

            <button
              type="button"
              className={`df-navbar-toggler${isOpen ? ' open' : ''}`}
              onClick={() => setIsOpen((prev) => !prev)}
              aria-expanded={isOpen}
              aria-controls="df-navbar-menu"
          ...
          <div
            id="df-navbar-menu"
            className={`df-navbar-collapse${isOpen ? ' show' : ''}`}
          >
            ...
            <LangSwitch lang={lang} onChange={setLang} className="df-lang-switch--mobile" />
          </div>
        </div>
      </nav>
      ...
      <section id="activities" className="df-section" style={{ background: 'var(--df-milk)' }}>
        <div className="container">
      ...
      <section id="alamana" className="df-section" style={{ background: '#fff' }}>
        <div className="container">
      ...
      <footer id="contacts" className="df-footer df-footer--solid">
        <div className="container">
          <div className="row g-4 g-lg-5">
            <div className="col-lg-4">
              <div className="df-footer-brand">
                <img src={logo} alt="" className="df-footer-logo" aria-hidden="true" />
                <h5>{t.footerAboutTitle}</h5>
              </div>
              <p className="df-footer-about">{t.footerAboutText}</p>
              <a
                href="https://t.me/"
            ...
            <div className="col-lg-5 col-md-6">
              <h5>{t.footerContactsTitle}</h5>
              <p className="df-footer-bank-label">{t.footerBankTitle}</p>
              <ul className="df-bank-details df-bank-details--light">
                <li>
          ...
          <p className="df-copyright df-copyright--light text-center mb-0">{t.copyright}</p>
        </div>
      </footer>
    </>
  );
